...
> 0202 年的前端已经不太需要考虑 ie 的兼容性问题。但是还是做个记录，技多不压身。

## WHAT

同样一份代码，有的浏览器效果正常，有的不正常。比如某些标签或者 CSS 的属性等，JS 同样有兼容问题，比如一些 JS 的 api 在某些或者某个版本的浏览器不能很好的运行。还有一些常用的前端库或者框架，有些浏览器也不能正常的运行。这些都算是兼容问题。
